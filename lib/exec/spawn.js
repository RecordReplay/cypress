...
function needsEverythingPipedDirectly(options) {
  return options.onTestOutput || isPlatform('win32');
}

function getStdio(options, needsXvfb) {
  if (needsEverythingPipedDirectly(options)) {
    return 'pipe';
...
module.exports = {
  ...
  start(args, options = {}) {
    ...
    _.defaults(options, {
      ...
      detached: false,
      stdio: getStdio(options, needsXvfb)
    });
    ...
    const spawn = (overrides = {}) => {
      return new Promise((resolve, reject) => {
        ...
        if (child.stdout) {
          if (options.onTestOutput) {
            debug('capturing child STDOUT to process STDOUT');
            child.stdout.on('data', data => {
              options.onTestOutput(data, false);
            });
          } else {
        ...
        if (child.stderr) {
          ...
          child.stderr.on('data', data => {
            ...
            if (options.onTestOutput) {
              options.onTestOutput(data, true);
            } else {
