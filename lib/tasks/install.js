...
const { ensurePlaywrightBrowsersInstalled } = require("@recordreplay/recordings-cli");
...
const start = (options = {}) => {
  ...
  return validateOS().then(isValid => {
    ...
    logger.log("Installing replay browser");
    await ensurePlaywrightBrowsersInstalled("gecko");
    logger.log("Replay browser installed");
  }).then(() => {
